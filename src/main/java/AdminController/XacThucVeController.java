package AdminController;

import java.io.IOException;
import java.util.ArrayList;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import KhachHangModal.KhachHang;
import KhachHangModal.KhachHangBo;
import LichSuDatVeModal.LichSuDatVe;
import LichSuDatVeModal.LichSuDatVeBo;
import VeXemPhimModal.VeXemPhimBo;

/**
 * Servlet implementation class XacThucVeController
 */
@WebServlet("/XacThucVeController")
public class XacThucVeController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public XacThucVeController() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		HttpSession session = request.getSession();
		VeXemPhimBo veBo = new VeXemPhimBo();
		KhachHangBo khBo = new KhachHangBo();
		LichSuDatVeBo lcBo = new LichSuDatVeBo();
		ArrayList<LichSuDatVe> ds = new ArrayList<LichSuDatVe>();
		ds = lcBo.getVeChuaThanhToan();
		session.setAttribute("VeChuaThanhToan", ds);
		if(request.getParameter("buttonThanhToan")!=null) {
			int mave = Integer.parseInt(request.getParameter("MaVe"));
			veBo.update(mave);
			ds = lcBo.getVeChuaThanhToan();
			session.setAttribute("VeChuaThanhToan", ds);
		}
		ArrayList<KhachHang> dskh = khBo.getKhachHang();
		session.setAttribute("dsKhachHang", dskh);
		RequestDispatcher rd = request.getRequestDispatcher("XacThucVe.jsp");
		rd.forward(request, response);
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

}
