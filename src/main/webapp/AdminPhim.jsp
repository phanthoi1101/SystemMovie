...
<body>
	...
	<div class="pb-4">
   <form id="frmSearchInput" action="PhimController" method="get">
       ...
       <div class="input-group-btn" >
           ...
           <a href="XoaSuaPhimController" class="btn btn-primary" style="margin-left:5px;">
              Thêm Phim
           </a>
       </div>
   </div>
	...
	<%ArrayList<Phim> ds = new ArrayList<Phim>();
	   ds = (ArrayList<Phim>)request.getAttribute("dsPhim");
	  int pagePhim = (int)request.getAttribute("page");
	  int rowCount = (int)request.getAttribute("pageCount");
	  int n=0;
	  if(ds!=null)
		   n = ds.size();
	%>
	<%if(n==0){ %>
		<div class="text-center">
			<p class="text-danger">Không có phim để hiển thị</p>
		</div>
	<%}else{ %>	
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-striped">
            ...
            <tbody>
               <%for(int i = 0; i < n ; i++){ %>
               		<tr>  
               		    <form action="XoaSuaPhimController" method="get">
               			<input type="hidden" name="maPhim" value="<%=ds.get(i).getMaPhim()%>">               
                        <td><%=ds.get(i).getTenPhim()%></td>
                        <td><%=ds.get(i).getDaoDien()%></td>
                        <td><%=ds.get(i).getDienVien()%></td>
                        <td><%=ds.get(i).getNgayPhatHanh().toString()%></td>
                        <td><%=ds.get(i).getThoiLuong()%></td>
                        <td class="text-right">
                        <input type="hidden" name="masach" value="">
                        <button class="btn btn-sm btn-info" name="btnEdit" value="<%=ds.get(i).getMaPhim()%>"><i class="fa fa-edit"></i></button>
...
<%} %>
	<form action="PhimController" method="get">
