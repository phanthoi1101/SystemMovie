...
<body>
	...
	<div class="pb-4">
   <form id="frmSearchInput" action="LoaiController" method="get">
       ...
       <div class="input-group-btn" >
           ...
           <a href="BoSungLoaiController" class="btn btn-primary" style="margin-left:5px;">
              Thêm loại
           </a>
       </div>
   </div>
	...
	<%ArrayList<TheLoai> ds = new ArrayList<TheLoai>();
	   ds = (ArrayList<TheLoai>)request.getAttribute("dsLoai");
	  int pageLoai = (int)request.getAttribute("pageLoai");
	  int rowCount = (int)request.getAttribute("pageCountLoai");
	  int n=0;
	  if(ds!=null)
		   n = ds.size();
	%>
	<%if(n==0){ %>
		<div class="text-center">
			<p class="text-danger">Không có loai để hiển thị</p>
		</div>
	<%}else{ %>	
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-striped">
            ...
            <tbody>
               <%for(int i = 0; i < n ; i++){ %>
               		<tr>              
                        <td><%=ds.get(i).getMaTheLoai()%></td>
                        <td><%=ds.get(i).getTenTheLoai()%></td>
                        <td class="text-right">
                        <form action="XoaSuaLoaiController" method="post">
                        <input type="hidden" name="maLoai" value="<%=ds.get(i).getMaTheLoai()%>">
...
<%} %>
	<form action="LoaiController" method="get">
